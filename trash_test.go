...
func TestListPackages(t *testing.T) {
	assert := require.New(t)
	p := listPackages("github.com/burmilla/trash", "vendor")
	logrus.Debug(p)
	assert.Equal(3, len(p))
	assert.Contains(p, "github.com/burmilla/trash")
	assert.Contains(p, "github.com/burmilla/trash/util")
	assert.Contains(p, "github.com/burmilla/trash/conf")
}
